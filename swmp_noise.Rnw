...
% example from RKBMB
\centering\vspace*{\fill}
<<case_ex1, cache = T, fig.height = 7.5, fig.width = 5.5, out.width = '0.8\\textwidth', eval = T, echo = F, fig.cap = 'Example of metabolism (top) and \\ac{DO} time series (middle) before (observed) and after (detided) detiding with weighted regression. Tidal height colored by total photosynthetically active radiation (mmol m$^{-2}) is on the bottom plot. Results are for the Rookery Bay station in July, 2012 using a weighted regression with a window of eight days, six hours within each day, and tidal height proportion of one.'>>=
######
# RKBMB example
...
case <- 'RKBMB'
dat.rng<-as.Date(c('2012-07-01','2012-07-31')) 
...
% plots of metab for all cases, period of obs
\centering\vspace*{\fill}
<<case_ex2, cache = T, fig.height = 7.5, fig.width = 5.5, out.width = '0.8\\textwidth', eval = T, echo = F, fig.cap = ''>>=
######
# metab before/after, all case studies
