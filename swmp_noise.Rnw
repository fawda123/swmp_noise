...
<<do_sim, eval = T, echo = F, cache = T, fig.width = 8, fig.height = 7, eval.after='fig.cap', fig.cap = 'Example of creating a simulated \\ac{DO} time series.  Simulated values were estimated every thirty minutes for 31 days.  Yellow indicates daylight periods.'>>=
# create time vector
...
prd_tmp <- wtreg_fun(DO_sim)
prd_tmp$DO_res <- with(prd_tmp, DO_obs - DO_prd)

levs <- c('DO_obs', 'DO_prd', 'DO_nrm', 'DO_res', 'DO_dtd')
...
\centering\vspace*{\fill}
<<err_surf, fig.height = 6, fig.width = 6, cache = T, echo = F, eval = T, fig.cap = 'Examples of correlations (left) and errors (right) between detided and biological \\ac{DO} from weighted regression results.  Each tile represents a correlation or error value from results for a given combination of simulation parameters ($DO_{adv}$, $DO_{die}$, $\\epsilon_{pro}$, $\\epsilon_{obs}$) and selected window widths (two and ten days).  Results are for simulations using a diurnal tidal time series.'>>=
...
jitt <- rnorm(nrow(prd_tmp[[1]]), 0, 1e-10)
for(i in 1:length(prd_tmp)){
  
  x <- prd_tmp[[i]]
  
  dtd_cor <- cor(x$DO_bio + jitt, x$DO_dtd + jitt)

  dtd_err <- rmse(x$DO_bio + jitt, x$DO_dtd + jitt)
  
  errs_all <- rbind(errs_all, data.frame(dtd_cor, dtd_err))
  
  }

# reassign factor labels for bio amp and tidal assoc
to_plo <- data.frame(comb_tmp, errs_all)
labs <- paste('Bio Amp',unique(to_plo$bio_rng))
to_plo$bio_rng <- factor(to_plo$bio_rng, labels = labs)
labs <- paste('Assoc',unique(to_plo$tide_assoc))
to_plo$tide_assoc <- factor(to_plo$tide_assoc, labels = labs)
labs <- paste('Window',unique(to_plo$dec_win))
to_plo$dec_win <- factor(to_plo$dec_win, labels = labs)
...
p1 <- ggplot(to_plo, aes(x = factor(err_rng_pro), y = factor(err_rng_obs), 
    z = dtd_cor, fill = dtd_cor)) +
  geom_tile() + 
  facet_grid(dec_win + bio_rng ~ tide_assoc, scales = 'free', 
    labeller = plot_labeller) +
  scale_fill_gradientn(limits = c(0.7, 1), name = 'Correlation', 
...
p2 <- ggplot(to_plo, aes(x = factor(err_rng_pro), y = factor(err_rng_obs), 
  ...
  facet_grid(dec_win + bio_rng ~ tide_assoc, scales = 'free', 
    labeller = plot_labeller) +
  scale_fill_gradientn(limits = c(0, 1.05), name = 'RMSE', 
    colours = brewer.pal(11, 'Spectral')) +
  scale_x_discrete(expand = c(0,0)) + 
  scale_y_discrete(expand = c(0,0)) +
  xlab(expression(italic(epsilon [pro]))) +
...
\centering\vspace*{\fill}
<<extreme, eval = T, echo = F, cache = T, fig.width = 7, fig.height = 6.5, fig.cap = 'An extreme example of results from the weighted regression with decreasing window widths.  Results are for a simulated time series with a diurnal tidal component, no effect of tidal advection ($DO_{adv} = 0$), and an observed \\ac{DO} signal dominated only by a diel component ($DO_{bio} = 2$, $\\epsilon_{obs} = 0$, $\\epsilon_{pro} = 0$).  Window widths varied form 2 to 40 days.  See \\cref{do_tid,do_mtd,do_dtd,do_obs} for notation.'>>=
######
# examples of how detiding works
...
% example from SAPDC
\centering\vspace*{\fill}
<<case_ex1, cache = F, fig.height = 7.5, fig.width = 5.5, out.width = '0.8\\textwidth', eval = T, echo = F, fig.cap = 'Example of metabolism and \\ac{DO} time series before (observed) and after (detided) weighted regression. Results are for a ten day period in March for the Sapelo Island station.'>>=
######
# SAPDC example
...
met_subs <- met_ls[[paste(case, 'wtreg.RData', sep = '_')]]
inst_subs <- met_ls_inst[[paste(case, 'wtreg.RData', sep = '_')]]
...
% example from ELKVM
\centering\vspace*{\fill}
<<case_ex2, cache = F, fig.height = 7.5, fig.width = 5.5, out.width = '0.8\\textwidth', eval = T, echo = F, fig.cap = 'Example of metabolism and \\ac{DO} time series before (observed) and after (detided) weighted regression. Results are for a ten day period in March for the Elkhorn Slough station.'>>=
######
# ELKVM example
...
# subsets by case and date range
case <- 'ELKVM'
dat.rng<-as.Date(c('2012-07-10','2012-07-20')) 
...
% example from pdbby
\centering\vspace*{\fill}
<<case_ex3, cache = F, fig.height = 7.5, fig.width = 5.5, out.width = '0.8\\textwidth', eval = T, echo = F, fig.cap = 'Example of metabolism and \\ac{DO} time series before (observed) and after (detided) weighted regression. Results are for a ten day period in March for the Padilla Bay station.'>>=
######
# PDBBY example
...
<<dtd_perf, cache = F, echo = F, eval = T, results = 'asis'>>=

# uses 'mod_perf' from 'dep_data.rnw'
load('mod_perf.RData')

# calculate mean, se for dtd_cor and dtd_err
dtd_perf <- llply(names(mod_perf)[1:6],
  .fun = function(x) {
    ddply(
      mod_perf,
      .variables = x,
      .fun = function(y) {
        c(
          mean(y$dtd_cor),
          sd(y$dtd_cor)/sqrt(length(y)),
          mean(y$dtd_err),
          sd(y$dtd_err)/sqrt(length(y))
        )
    })}
  )
# multiply windows by two since value is half window width
dtd_perf[[6]]$dec_time <- 2*dtd_perf[[6]]$dec_time
...
names(dtd_perf) <- c('Parameter', 'Level', 'Mean cor', 'Se cor', 'Mean err', 'Se err')
...
Parms <- c('Tidal category', '$\\boldsymbol{DO_{die}}$', 
  ...
  'Window')
tab <- form_fun(dtd_perf[, -c(1:2)])
rows <- dtd_perf[, 2]
cap.val<-'Mean correlations and error estimates between detided and biological \\ac{DO} time series for different simulation parameters (tidal category, $DO_{die}$, $DO_{adv}$, $\\epsilon_{pro}$, $\\epsilon_{obs}$) and window widths (days).  Each value represents a combination of results from multiple simulations with the parameter value held constant for each row (e.g., row one is a summary of all simulations for which the tidal category was diurnal).'
...
#   insert.bottom = foot.val,
  ...
  caption.loc = 'top',
  rgroup = Parms,
  n.rgroup = c(rep(3, 5), 5),
  cgroup = c('Correlation', 'RMSE'),
  n.cgroup = c(2, 2),
  rowname = rows,
  colheads=rep(c('Mean','Std. Err.'),2),
  label = 'tab:dtd_perf'
  )
...
# case_regs <- list.files(getwd(), '_wtreg.RData')
# cor_res <- adply(matrix(case_regs),
#   1, 
#   .fun = function(x){
#   
#     # load wtreg data
...
#     dat_in <- met_ls[[x]]
#     
#     # melt for quicker analysis
#     dat_in <- melt(dat_in, 
#       measure.var = c('Pg', 'Rt', 'NEM', 'Pg_dtd', 'Rt_dtd', 'NEM_dtd'), 
#       id.var = c('Tide'))
#     
#     met_cor <- dlply(dat_in, 
#       .(variable), 
#       function(y) cor.test(y$value, y$Tide)
#       )
#     names(met_cor) <- c('Pg_obs', 'Rt_obs', 'NEM_obs', 'Pg_dtd', 'Rt_dtd', 
#       'NEM_dtd')
#     
...
#   })
# 
# cor_res$site <- factor(cor_res$X1, 
#   labels = gsub('_wtreg.RData', '', case_regs))
# cor_res$X1 <- NULL
# 
# names(cor_res) <- c('var', 'corr', 'pval', 'site')
# 
# cor_res$sub_var <- gsub('[A-Z,a-z]*_', '', cor_res$var)
# cor_res$var <- gsub('_obs|_dtd', '', cor_res$var)
# 
# save(cor_res, file = 'cor_res.RData')

load('cor_res.RData')

# subset all results for plotting
to_tab <- cor_res

to_tab <- dcast(cor_res, site + sub_var ~ var, value.var = 'corr')
...
to_tab <- to_tab[, c('site', 'sub_var', 'do', 'flux', 'Pg', 'Rt', 'NEM')]
...
pvals <- pvals[, c('site', 'sub_var', 'do', 'flux', 'Pg', 'Rt', 'NEM')]
...
cap.val<-'Correlations of tidal changes at each site with continuous \\ac{DO} observations, instantaneous \\ac{DO} flux, and metabolism estimates (gross production, respiration, and net metabolism) before (observed) and after detiding with weighted regression.  \\ac{DO} values are correlated with predicated tidal height at each observation, \\ac{DO} flux values are correlated with tidal change between observations, and daily integrated metabolism estimates are correlated with daily tidal range.'
...
latex( 
  ...
  insert.bottom = foot.val,
  caption = cap.val,
  colheads = c('DO', 'DO flux', 'Pg\\textsuperscript{\\textit{a}}', 'Rt', 'NEM'),
  caption.loc = 'top',
...
rows <- to_tab[, 2]
cap.val<-'Metabolism estimates (gross production, respiration, and net metabolism) for case studies using \\ac{DO} time series before (observed) and after (detided) application of weighted regression.  Means and standard errors are based on daily integrated metabolism estimates.  Anomalous values are the proportion of metabolism estimates that were negative for gross production and positive for respiration.'
...
latex( 
  ...
  caption = cap.val,
  caption.loc = 'top',
  rgroup = gsub('_wtreg.RData', '', unique(to_tab$Site)),
  n.rgroup = rep(2, 4),
