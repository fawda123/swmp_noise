...
{\it Text was added to elaborate this point, both in the introduction and the abstract.

Line 100: `This approach offers a distinct advantage for estuaries where the magnitude of tidal effects on water quality observations can be severe. The weighted regression approach targets the tidal component as a specific parameter during model fitting, which allows the ability to isolate the biological component of the dissolved oxygen time series.  As a result, the weighted regression approach can preserve the true biological signal in the output rather'
