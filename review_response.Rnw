...
Although we mentioned these points previously (e.g., lines 431, 521) we added additional explanation based on the reviewer’s comments. Specifically, we added text to the comments and recommendations section (starting on line 527) to emphasize how and why the model can effectively characterize and remove the tidal signal, emphasizing that the model can work well in a variety of tidal regimes.
...
\frac{\delta DO_{adv}}{\partial t} = \frac{\partial DO}{\partial x} \cdot \frac{\partial x}{\partial t}
\end{equation}
